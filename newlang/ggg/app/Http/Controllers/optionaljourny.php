...
class optionaljourny extends Controller
{
    public function AddOptionalJourny(RequestsOptionaljourny $journy) {
        ...
        $continent = continent::where('continents_Name', $journy->continents_id)->first();
        if (!$continent) {
        ...
        $season = season::where('season_Name', $journy->season_id)->first();
        if (!$season) {
        ...
        $section = section::where('section_Name', $journy->section_id)->first();
        if (!$section) {
        ...
        $type_ticket = type_ticket::where('type', $journy->type_ticket_id)->first();
        if (!$type_ticket) {
        ...
        $photos = [];
        for ($i = 1; $i <= 3; $i++) {
            $photoKey = 'journy_photo' . $i;
            if ($journy->hasFile($photoKey)) {
                $photo = $journy->file($photoKey);
                $photoName = time() . '_' . $i . '.' . $photo->getClientOriginalExtension();
                $photoPath = 'photos/' . $photoName;
                $photo->move(public_path($photoPath), $photoName);
                $photos[$photoKey] = $photoPath;
            }
        }

        $addjourny->fill($photos);
        $addjourny->save();
    ...
    public function EditOptionalJourny(Request $request, $journyId)
{
    ...
    $photos = ['journey_photo1', 'journey_photo2', 'journey_photo3', 'photo1', 'photo2', 'photo3'];
foreach ($photos as $photo) {
    if ($request->hasFile($photo)) {
        $file = $request->file($photo);
        $filename = $file->getClientOriginalName();
        $photoPath = $file->storeAs('photos', $filename, 'public');
        $journy->$photo = $photoPath;
    }
}
$journy->save();
...
public function getoptionaltripschosen($id){
    $optional=ModelsOptionaljourny::find($id);
