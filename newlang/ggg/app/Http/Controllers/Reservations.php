...
public function constTripReservation(Request $request, $userId, $constTripId)
{
    ...
    $constTripReservation->user_id =  $user->id;
...
$tansportationname=transportation::find($consttrip->transportation_id);
    return response()->json([
        ...
        'transportation:' => $tansportationname->transportation_Name,
...
public function reservationsForUser($userID)
{
    ...
    $optionaljournyReservation = optionaljournyReservation::where('user_id', $userID)->get();
    $consttripReservation = const_trip_reservation::where('user_id', $userID)->get();
    $service = service::where('user_id', $userID)->get();

    $hasReservations = false;
    $response = ['Your reservations:'];

    if (!$optionaljournyReservation->isEmpty()) {
        $hasReservations = true;
        $response[] = 'Optional Journy Reservations:';
        $response[] = $optionaljournyReservation;
    } else {
        $response[] = 'Optional Journy Reservations:';
        $response[] = 'No Optional Journy Reservations found.';
    }

    if (!$consttripReservation->isEmpty()) {
        $hasReservations = true;
        $response[] = 'Const Trip Reservations:';
        $response[] = $consttripReservation;
    } else {
        $response[] = 'Const Trip Reservations:';
        $response[] = 'No Const Trip Reservations found.';
    }

    if (!$service->isEmpty()) {
        $hasReservations = true;
        $response[] = 'Service Reservations:';
        $response[] = $service;
    } else {
        $response[] = 'Service Reservations:';
        $response[] = 'No Service Reservations found.';
    }

    if (!$hasReservations) {
        return response()->json([ 'No reservations found for this user.'], 404);
    }
