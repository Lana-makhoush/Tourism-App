...
if(!$tripSchadual){
    return response()->json(['message' => 'Trip Schadual not found!'], 404);
       ...
       $constjourny->destination_en = $request->destination_en;
        ...
        $season = season::where('season_Name', $request->season_id)->first();
        if (!$season) {
        ...
        foreach ($photos as $photo) {
            if ($request->hasFile($photo)) {
                ...
                $constjourny->$photo = $path;
            }
        }
   $constjourny->save();

        return response()->json(['message' => 'Journey details added successfully'], 200);
    }
    function EditConstJourny(Request $request, $constJournyId, $hotelId, $transportationId,$tripschadualId)
{
    ...
    return response()->json(['message' => 'Journey updated successfully']);
}
...
$constId=const_trip::find($constId);
