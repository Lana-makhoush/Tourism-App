    ...
    public function AddFoodToTheMenu(Request $request)
{
    ...
    $hotel = hotel::where('hotel_Name', $request->hotel_id)->first();
    ...
    if ($request->hasFile('Fphoto')) {
        $Fphoto = time() . '.' . $request->file('Fphoto')->getClientOriginalExtension();
        $request->file('Fphoto')->move(public_path('photos/F'), $Fphoto);
        $food->Fphoto = 'photos/F/' . $Fphoto;
    }
    $food->S_dish_en = $request->S_dish_en;
    $food->S_dish_ar = $request->S_dish_ar;
    if ($request->hasFile('Sphoto')) {
        $Sphoto = time() . '.' . $request->file('Sphoto')->getClientOriginalExtension();
        $request->file('Sphoto')->move(public_path('photos/S'), $Sphoto);
        $food->Sphoto = 'photos/S/' . $Sphoto;
    }
    ...
    if ($request->hasFile('Tphoto')) {
        $Tphoto = time() . '.' . $request->file('Tphoto')->getClientOriginalExtension();
        $request->file('Tphoto')->move(public_path('photos/T'), $Tphoto);
        $food->Tphoto = 'photos/T/' . $Tphoto;
    }
    ...
    if ($request->hasFile('FOphoto')) {
        $FOphoto = time() . '.' . $request->file('FOphoto')->getClientOriginalExtension();
        $request->file('FOphoto')->move(public_path('photos/FO'), $FOphoto);
        $food->FOphoto = 'photos/FO/' . $FOphoto;
    }
...
public function updateFoodMenu(Request $request, $foodId)
{
    ...
    $food->type_ar = $request->input('type_en', $food->type_ar);

    $food->F_dish_en = $request->input('F_dish_en', $food->F_dish_en);
    $food->F_dish_ar = $request->input('F_dish', $food->F_dish_ar);
    $food->F_price = $request->input('F_price', $food->F_price);
    if ($request->hasFile('Fphoto')) {
        if ($food->Fphoto) {
            Storage::disk('public')->delete($food->Fphoto);
        }
        $Fphoto = time() . '.' . $request->file('Fphoto')->getClientOriginalExtension();
        $request->file('Fphoto')->move(public_path('photos/F'), $Fphoto);
        $food->Fphoto = 'photos/F/' . $Fphoto;
    ...
    if ($request->hasFile('Sphoto')) {
        if ($food->Sphoto) {
            Storage::disk('public')->delete($food->Sphoto);
        }
        $Sphoto = time() . '.' . $request->file('Sphoto')->getClientOriginalExtension();
        $request->file('Sphoto')->move(public_path('photos/S'), $Sphoto);
        $food->Sphoto = 'photos/S/' . $Sphoto;
    ...
    if ($request->hasFile('Tphoto')) {
        if ($food->Tphoto) {
            Storage::disk('public')->delete($food->Tphoto);
        }
        $Tphoto = time() . '.' . $request->file('Tphoto')->getClientOriginalExtension();
        $request->file('Tphoto')->move(public_path('photos/T'), $Tphoto);
        $food->Tphoto = 'photos/T/' . $Tphoto;
    ...
    if ($request->hasFile('FOphoto')) {
        if ($food->FOphoto) {
            Storage::disk('public')->delete($food->FOphoto);
        }
        $FOphoto = time() . '.' . $request->file('FOphoto')->getClientOriginalExtension();
        $request->file('FOphoto')->move(public_path('photos/FO'), $FOphoto);
        $food->FOphoto = 'photos/FO/' . $FOphoto;
